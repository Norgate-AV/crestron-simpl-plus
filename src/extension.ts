...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log(
        'Congratulations, your extension "crestron-simpl-plus" is now active!',
    );
    ...
    let previewUri = vscode.Uri.parse(
        "simpl-visualize://authority/simpl-visualize",
    );
    ...
    let series3_compile = vscode.commands.registerCommand(
    ...
    let series4_compile = vscode.commands.registerCommand(
        "extension.simplCC_Series4",
        () => {
            processSimpl("\\target series4");
        },
    );

    let series3and4_compile = vscode.commands.registerCommand(
        "extension.simplCC_Series3and4",
        () => {
            processSimpl("\\target series3 series4");
        },
    );

    let series2and3and4_compile = vscode.commands.registerCommand(
        "extension.simplCC_Series2and3and4",
        () => {
            processSimpl("\\target series2 series3 series4");
        },
    );

    let series2and3_compile = vscode.commands.registerCommand(
        "extension.simplCC_Series2and3",
        () => {
            processSimpl("\\target series2 series3");
        },
    );

    let series3_compileAll = vscode.commands.registerCommand(
        "extension.simplCC_Series3All",
        () => {
            let foundFiles = vscode.workspace.findFiles("*.usp");
            let term = vscode.window.createTerminal(
                "simplCC",
                vscode.workspace.getConfiguration("simpl").terminalLocation,
            );
    ...
    let simpl_visualize = vscode.commands.registerCommand(
        "extension.simpl_visualize",
        () => {
    ...
    let help_command = vscode.commands.registerCommand(
        "extension.simpl_help",
        () => {
            let helpLocation =
                vscode.workspace.getConfiguration("simpl").helpLocation;
    ...
    let open_api = vscode.commands.registerCommand(
        "extension.simplCC_API",
        () => {
            let editor = vscode.window.activeTextEditor;
            if (editor) {
...
class SimplCompiler {
    ...
    constructor() {
        this.compilerPath =
            '"' + vscode.workspace.getConfiguration("simpl").compiler + '"';
    ...
    public buildCommand(args: string): string {
        ...
        return this.compilerPath + " \\rebuild " + filepathConcat + " " + args;
    }
}

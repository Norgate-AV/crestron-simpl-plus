...
export class API {
            ...
            files.forEach((e) => {
                ...
                if (this.checkRecent(e)) {
                    ...
                    vscode.window.showErrorMessage(
                        "We were unable to find or generate the API file: " +
                            apiPath,
                    );
                }
            });
    ...
    private getLibraryNames(): RegExpMatchArray | null {
        return this.fileText.match(
            /#USER_SIMPLSHARP_LIBRARY\s*?"([\S\s]*?)"/gim,
        );
    }
    ...
    private filterLibraryNames(arr: RegExpMatchArray): string[] {
        let results: string[] = [];
        arr.forEach((e) => {
            let match = e.match(/#USER_SIMPLSHARP_LIBRARY\s*?"([\S\s]*?)"/im);
            if (match) {
